...
/**
 * A small north-arrow compass rose — a standard real-map convention, rendered
 * in both quiet and active modes now that the basemap is real satellite
 * imagery. Plain HTML/SVG overlay (not a MapLibre layer): it's static, not
 * data-driven, so it doesn't need a source/layer round-trip. White strokes +
 * a drop-shadow (see tropical-map.css) keep it legible over imagery of any
 * brightness.
 */
  ...
  return (
    ...
    >
      <g transform="translate(30,32)" stroke="#fff" fill="none">
        ...
        <text y="-26" textAnchor="middle" fontSize="10" fill="#fff" stroke="none">
          N
        </text>
      </g>
    </svg>
...
export default function StormMap({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // invoke, a map that already fired 'load' would otherwise leave `loaded`
      // true while the replacement map is still building its style, and every
      // data effect below would no-op against a source that doesn't exist yet.
      setLoaded(false);
    };
  }, []);
  ...
  useEffect(() => {
    ...
    otherStormMarkersRef.current = otherStorms.map((s) => {
      ...
      el.className = "tmap-label tmap-storm-label";
  ...
  useEffect(() => {
    ...
    outlookMarkersRef.current = (geo.outlook?.features ?? [])
      .map((f) => {
        ...
        const el = document.createElement("div");
        el.className = "tmap-label tmap-outlook-label";
  ...
  return (
    <div className="tw-map-frame relative h-full w-full overflow-hidden bg-[#0b1b28]">
      {/* Sized with h/w rather than absolute inset-0: MapLibre stamps its own
          `.maplibregl-map { position: relative }` on this element from
          maplibre-gl.css, which loads after the Tailwind utilities and wins on
          equal specificity — so an `absolute` utility here silently collapses
          the container to zero height and the map renders at canvas's 300px
          default. */}
      ...
      <div className="absolute bottom-0 right-0 z-10 bg-black/45 px-1.5 py-0.5 text-[10px] text-white/80">
        {ESRI_ATTRIBUTION}
      </div>
      {layers.satellite && geo.satellite && (
        <a
          className="absolute bottom-0 left-0 z-10 bg-black/45 px-1.5 py-0.5 text-[10px] text-white/80 hover:text-white"
