 ...
 * captured advisory-016 snapshot (public/tropical/demo/bertha/manifest.json)
 * and `demo=ida` the real Hurricane Ida historical sample
 * (public/tropical/demo/ida/manifest.json) — both have `storms[0].files`
 * values pre-rewritten to `<variant>/<name>.geojson` (relative to
 * `DEMO_BASE`), so no base-URL branching is needed elsewhere: `base` in
 * {@link useDashboard} stays `DEMO_BASE` for every demo variant, and the
 * manifest's own `files` map does all the path resolution.
 *
 * Any demo value other than "quiet"/"bertha" falls back to the Ida sample.
 */
