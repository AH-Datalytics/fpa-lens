...
/**
 * Plain-English gloss for each guidance code, surfaced as a hover tooltip in
 * the map's model list and the intensity panel's legend. The a-deck labels
 * alone ("EGRI", "HCCA", "CMC (interpolated)") mean nothing to a general
 * audience, and the whole point of showing model spread is that a reader can
 * tell whose forecast they're looking at.
 *
 * "Interpolated" siblings are the same model shifted forward to line up with
 * the current advisory time — guidance arrives hours after its own start time,
 * so NHC adjusts it rather than plotting a stale start point.
 */
export const MODEL_DESCRIPTIONS: Record<string, string> = {
  OFCL: "The National Hurricane Center's official forecast.",
  AVNO: "GFS — NOAA's global weather model.",
  EMXI: "ECMWF — the European global model, historically among the strongest track performers.",
  EGRR: "UKMET — the UK Met Office global model.",
  EGRI: "UKMET, shifted to line up with the current advisory time.",
  CMC: "CMC — the Canadian global model.",
  CMCI: "CMC, shifted to line up with the current advisory time.",
  NVGM: "NAVGEM — the U.S. Navy's global model.",
  HFSA: "HAFS-A — NOAA's high-resolution, hurricane-specific model.",
  HFSB: "HAFS-B — a second configuration of NOAA's hurricane-specific model.",
  HWRF: "HWRF — NOAA's earlier hurricane-specific model.",
  HMON: "HMON — a NOAA hurricane-specific model run alongside HWRF.",
  ...
  GFEX: "The average of the GFS ensemble members.",
  DSHP: "SHIPS — statistical intensity guidance. Intensity only, no track.",
  LGEM: "LGEM — statistical intensity guidance. Intensity only, no track.",
  IVCN: "A consensus of the intensity models. Intensity only, no track.",
};

/** Tooltip text for a model code, falling back to a generic gloss. */
export function modelDescription(code: string, label?: string): string {
  return (
    MODEL_DESCRIPTIONS[code] ??
    `${label ?? code} — forecast guidance from the National Hurricane Center's model suite.`
  );
}

export const ENSEMBLE_DESCRIPTION =
  "One model run many times with slightly different starting conditions. The width of the spread is the forecast uncertainty.";
