...
// src/lib/tropical/__tests__ -> repo root -> public/tropical/demo
const DEMO_ROOT = join(__dirname, "..", "..", "..", "..", "public", "tropical", "demo");

const manifest: Manifest = JSON.parse(
  readFileSync(join(DEMO_ROOT, "ida", "manifest.json"), "utf-8")
);
...
describe("Ida flagship sample manifest", () => {
  it("is served from the URL manifestUrl('ida') resolves", () => {
    expect(manifestUrl("ida")).toBe("/tropical/demo/ida/manifest.json");
  });
  ...
  it("carries observed history and a time-matched GOES overlay", () => {
    ...
    expect(
      existsSync(join(DEMO_ROOT, ida.satellite!.image))
    ).toBe(true);
  ...
  it("carries an advisory-matched archived NEXRAD image for every frame", () => {
    for (const frame of manifest.storms[0].advisories ?? []) {
      ...
      expect(
        frame.radar && existsSync(join(DEMO_ROOT, frame.radar.image))
      ).toBe(true);
    }
  });
  ...
  it("ships every file referenced by every advisory frame", () => {
    for (const frame of manifest.storms[0].advisories ?? []) {
      for (const path of Object.values(frame.files)) expect(existsSync(join(DEMO_ROOT, path))).toBe(true);
      expect(frame.satellite && existsSync(join(DEMO_ROOT, frame.satellite.image))).toBe(true);
    }
  });
  ...
  it("removes non-Ida systems from the basin-wide wind-probability archives", () => {
    for (const frame of manifest.storms[0].advisories ?? []) {
      for (const path of [frame.files.windprob, frame.files.windprob50, frame.files.windprob64]) {
        const field: GeoJSON.FeatureCollection = JSON.parse(readFileSync(join(DEMO_ROOT, path!), "utf-8"));
