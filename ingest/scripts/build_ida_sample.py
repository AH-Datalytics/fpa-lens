...
def _build_advisory(advisory: dict, adeck_full: str, best_track: dict) -> dict:
    ...
    # Clip to the advisory being replayed, exactly as the live pipeline clips to
    # the current advisory -- the replay has the same 3h synoptic-vs-issuance
    # offset by construction, so without this its tracks would start behind the
    # storm while a live storm's start at it.
    #
    # NOTE: the checked-in fixtures under public/tropical/demo/ predate this and
    # are NOT regenerated by this change; they only pick it up if this builder is
    # re-run (which re-fetches every archived NHC product).
    parsed = adeck.parse_adeck("\n".join(adeck_lines), reference_time=advisory["issued"])
