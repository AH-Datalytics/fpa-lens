...
// breaks under Turbopack (see src/components/tropical/StormMap.tsx's comment
// for the full root-cause) — the fix is a static copy of the worker + its
// shared chunk in public/. If maplibre-gl is ever upgraded without re-copying
...
//
// Usage: node scripts/check-maplibre-worker-sync.mjs
...
for (const { installed, copied } of PAIRS) {
  ...
  if (installedContent !== copiedContent) {
    ...
    console.error("  breaks under Turbopack. See src/components/tropical/StormMap.tsx's comment.");
    console.error(`  FIX: cp "${installed}" "${copied}"`);
    ok = false;
  }
}
