...
- **Official overrides for closed years (2022-2025)**: Jamal's team re-reviewed the records and supplied authoritative figures that supersede the event-log-derived split (row-level tagging diverged from the final classification, notably 2022 and 2024). Two override tables in `extractSafetyData.py`, applied in `build_output`:
  - `OFFICIAL_YEARLY_OVERRIDES` — annual accidents/incidents/injuries (Jamal, June 2026).
  - `OFFICIAL_MONTHLY_OVERRIDES` — the per-month accident/incident split from Jamal's June 2026 "Lens Monthly Safety Numbers" workbook, so the per-month chart reconciles to those annual totals. Every closed year sums exactly to its yearly override. No-fault monthly counts stay event-log-derived (not part of his breakdown); injuries are tracked only at the year level. The live year (2026) is intentionally excluded from both so weekly refreshes keep updating it (currently event-log-derived through May; Jamal's workbook also lists 2026 through June, but we don't freeze the live year).
