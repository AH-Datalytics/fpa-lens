...
- **Structure:** `src/payload.config.ts`; collections `Users` (auth, `admin`/`editor` roles), `Media`, `StaffMembers`; globals `SiteSettings`, `HomeContent`, and the per-page `pages/*` globals. Route groups: existing site under `src/app/(frontend)/`, admin under `src/app/(payload)/`. `package.json` is `"type": "module"` (required for the Payload CLI); after changing admin components/config run `CMS_MEDIA_BLOB_TOKEN=x npx payload generate:importmap` and `generate:types`. **Always set `CMS_MEDIA_BLOB_TOKEN` (any value) when regenerating the importmap** -- without it the blob plugin is disabled and `importMap.js` loses `VercelBlobClientUploadHandler`. Prod enables the blob plugin, so a stripped map mismatches at runtime and the **entire prod admin renders blank** (login + reset included, 0 console errors). If you must regen without the token, restore that entry before committing. And after any admin/config deploy, browser-check `/admin` on prod, not just public pages.
