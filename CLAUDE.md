...
- Per-reach acreages and per-month cycle percentages are hand-entered in `src/data/grassCutting.ts` from the input-template workbook. Current values reflect April 2026 finals as of the May 7 input template:
  ...
  - EJLD `hasReportedData=false` — Jeff getting verbal from team to backfill April actuals.
...
- Outstanding:
  ...
  - **EJLD April actuals**: no weekly C1/C2 percentages received; cards show "Awaiting weekly update." Jeff getting verbal from team May 8, 2026 — he's "pretty sure we cut all reaches twice."
