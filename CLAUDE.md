...
- **Seed:** `scripts/seed-cms.ts` (idempotent, `payload run`) populates content + the 4 editors from `siteData.ts`. Add `SEED_DEV_ADMIN=1` for a local `dev@fpalens.local`/`devpassword123` admin (never in prod). Seed prod against Neon's **direct/unpooled** string (the pooled/sensitive vars can't be pulled via CLI). `scripts/set-password.ts` grants initial access before the Resend "Forgot password" flow.
- **Editors are managed in-panel** (Users collection); invites/resets email via the existing `RESEND_API_KEY`.
