...
- **Alert email policy** (`src/lib/riskAlertDecision.ts`, used by the 5-min cron `api/cron/collect-forecasts`): decisions key off the last *notified* level (persisted only when a send succeeds), NOT the last observed level — so an "all clear" fires solely to clear an alert we actually sent (fixes repeated all-clear spam from transient spikes). Only ORANGE/RED are alert-worthy; YELLOW is routine and never emails (alerts fire crossing into/out of the ORANGE+ band). RED always alerts on the way up regardless of trend. Otherwise trend-gated (skip improving escalations / worsening de-escalations). Pure logic is unit-tested (`riskAlertDecision.test.ts`).
